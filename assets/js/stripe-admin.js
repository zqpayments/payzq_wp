		...
		init: function() {
			$( document.body ).on( 'change', '#woocommerce_payzq_testmode', function() {
				var test_secret_key = $( '#woocommerce_payzq_test_secret_key' ).parents( 'tr' ).eq( 0 ),
					test_publishable_key = $( '#woocommerce_payzq_test_publishable_key' ).parents( 'tr' ).eq( 0 ),
					live_secret_key = $( '#woocommerce_payzq_secret_key' ).parents( 'tr' ).eq( 0 ),
					live_publishable_key = $( '#woocommerce_payzq_publishable_key' ).parents( 'tr' ).eq( 0 );
			...
			$( '#woocommerce_payzq_stripe_checkout' ).change( function() {
				if ( $( this ).is( ':checked' ) ) {
					...
					$( '#woocommerce_payzq_request_payment_api' ).closest( 'tr' ).hide();
				} else {
					...
					$( '#woocommerce_payzq_request_payment_api' ).closest( 'tr' ).show();
				}
			}).change();

			// Toggle Apple Pay settings.
			$( '#woocommerce_payzq_apple_pay' ).change( function() {
				if ( $( this ).is( ':checked' ) ) {
					$( '#woocommerce_payzq_apple_pay_button, #woocommerce_payzq_apple_pay_button_lang' ).closest( 'tr' ).show();
				} else {
					$( '#woocommerce_payzq_apple_pay_button, #woocommerce_payzq_apple_pay_button_lang' ).closest( 'tr' ).hide();
				}
			}).change();

			// Validate the keys to make sure it is matching test with test field.
			$( '#woocommerce_payzq_secret_key, #woocommerce_payzq_publishable_key' ).on( 'input', function() {
				var value = $( this ).val();
			...
			$( '#woocommerce_payzq_test_secret_key, #woocommerce_payzq_test_publishable_key' ).on( 'input', function() {
				var value = $( this ).val();
