/* global wc_payzq_params */
Stripe.setPublishableKey( wc_payzq_params.key );
	...
	var wc_stripe_form = {
		...
		onError: function( e, responseObject ) {
			...
			if (
				...
				message = wc_payzq_params.invalid_request_error;
			}

			if ( 'card_error' === responseObject.response.error.type && wc_payzq_params.hasOwnProperty( responseObject.response.error.code ) ) {
				message = wc_payzq_params[ responseObject.response.error.code ];
			}
		...
		onSubmit: function( e ) {
			...
			if ( wc_stripe_form.isStripeChosen() && ! wc_stripe_form.hasToken() ) {
				...
				var card       = $( '#stripe-card-number' ).val(),
					...
					first_name = $( '#billing_first_name' ).length ? $( '#billing_first_name' ).val() : wc_payzq_params.billing_first_name,
					last_name  = $( '#billing_last_name' ).length ? $( '#billing_last_name' ).val() : wc_payzq_params.billing_last_name,
				...
				} else if ( wc_payzq_params.billing_address_1 ) {
					data.address_line1   = wc_payzq_params.billing_address_1;
					data.address_line2   = wc_payzq_params.billing_address_2;
					data.address_state   = wc_payzq_params.billing_state;
					data.address_city    = wc_payzq_params.billing_city;
					data.address_zip     = wc_payzq_params.billing_postcode;
					data.address_country = wc_payzq_params.billing_country;
				}
		...
		onStripeResponse: function( status, response ) {
			if ( response.error ) {
				...
				if ( 'no' === wc_payzq_params.allow_prepaid_card && 'prepaid' === response.card.funding ) {
					response.error = { message: wc_payzq_params.no_prepaid_card_msg };
