...
class WC_Stripe_Customer {
	...
	public function get_customer_data() {
		if ( empty( $this->customer_data ) && $this->get_id() && false === ( $this->customer_data = get_transient( 'stripe_customer_' . $this->get_id() ) ) ) {
			$response = WC_PayZQ_API::request( array(), 'customers/' . $this->get_id() );
	...
	public function create_customer( $args = array() ) {
		...
		$response = WC_PayZQ_API::request( $args, 'customers' );

		if ( is_wp_error( $response ) ) {
			return $response;
		} elseif ( empty( $response->id ) ) {
			return new WP_Error( 'stripe_error', __( 'Could not create Stripe customer.', 'woocommerce-gateway-payzq' ) );
		}
		...
		do_action( 'woocommerce_payzq_add_customer', $args, $response );
	...
	public function add_card( $token, $retry = true ) {
		...
		$response = WC_PayZQ_API::request( array(
			'source' => $token,
		...
		if ( is_wp_error( $response ) ) {
			...
			return new WP_Error( 'error', __( 'Unable to add card', 'woocommerce-gateway-payzq' ) );
		}
		...
		do_action( 'woocommerce_payzq_add_card', $this->get_id(), $token, $response );
	...
	public function delete_card( $card_id ) {
		$response = WC_PayZQ_API::request( array(), 'customers/' . $this->get_id() . '/sources/' . sanitize_text_field( $card_id ), 'DELETE' );
