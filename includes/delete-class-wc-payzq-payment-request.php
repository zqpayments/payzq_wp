 ...
 *
 * @package woocommerce_payzq/Classes/Payment_Request
 * @since   3.1.0
 * @version 3.1.0
 */
...
/**
 * WC_PayZQ_Payment_Request class.
 */
class WC_PayZQ_Payment_Request {
	...
	protected function is_activated() {
		$options             = get_option( 'woocommerce_payzq_settings', array() );
	...
	public function scripts() {
		...
		wp_enqueue_script( 'wc-stripe-payment-request', plugins_url( 'assets/js/payment-request' . $suffix . '.js', WC_PAYZQ_MAIN_FILE ), array( 'jquery', 'stripe' ), WC_PAYZQ_VERSION, true );
		...
		wp_localize_script(
			...
			array(
				...
				'i18n'     => array(
					'no_prepaid_card'  => __( 'Sorry, we\'re not accepting prepaid cards at this time.', 'woocommerce-gateway-payzq' ),
					/* translators: Do not translate the [option] placeholder */
					'unknown_shipping' => __( 'Unknown shipping option "[option]".', 'woocommerce-gateway-payzq' ),
				),
			)
		);
	}
	...
	public function ajax_get_cart_details() {
		...
		$data = array(
			...
			'order_data'        => array(
				'total' => array(
					'label'  => __( 'Total', 'woocommerce-gateway-payzq' ),
					'amount' => array(
	...
	public function ajax_create_order() {
		if ( WC()->cart->is_empty() ) {
			wp_send_json_error( __( 'Empty cart', 'woocommerce-gateway-payzq' ) );
		}
	...
	protected function compute_display_items( $shipping_method_id ) {
		...
		$items = array(
			...
			array(
				'label' => __( 'Subtotal', 'woocommerce-gateway-payzq' ),
				'amount' => array(
		...
		if ( 0 < $tax_total ) {
			$items[] = array(
				'label'  => __( 'Tax', 'woocommerce-gateway-payzq' ),
				'amount' => array(
					'currency' => $currency,
					'value'    => $tax_total,
				),
